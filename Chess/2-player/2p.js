...
function endgame() {
    ...
    document.getElementById('announcement').innerHTML = `Le joueur ${document.getElementById("player").outerHTML} a gagné !`;
...
function back(c) {
    ...
    if (m.promotion) {
        p.classList[0] = 'Pawn'
        p.src = `../Images/pawn_${p.classList[2].split('_')[0]}.png`
    }
};
function next(c) {
    if (!c.contains('allowed')) return;
    ...
    let oldLine = 0;
    var taken = false;
    let pastPiece = { lastId: '', position: '', id: '', taken: null, promotion: null };
    if (m === '1/2-1/2' || m === '1-0' || m === '0-1') {
        ...
        if (m == 'O-O') {
            var a = document.getElementById(`5,${currentMove % 2 == 0 ? '8' : '1'}`);
            var b = document.getElementById(`8,${currentMove % 2 == 0 ? '8' : '1'}`);
            if (a && b) {
                a.setAttribute('style', `transform:translate(480px,${currentMove % 2 == 0 ? 0 : 560}px);`);
                b.setAttribute('style', `transform:translate(400px,${currentMove % 2 == 0 ? 0 : 560}px);`);
                a.id = `7,${a.id.split(',')[1]}`;
                b.id = `6,${b.id.split(',')[1]}`;
            };
        } else {
            var a = document.getElementById(`5,${currentMove % 2 == 0 ? '8' : '1'}`);
            var b = document.getElementById(`1,${currentMove % 2 == 0 ? '8' : '1'}`);
            if (a && b) {
                a.setAttribute('style', `transform:translate(80px,${currentMove % 2 == 0 ? 0 : 560}px);`);
                b.setAttribute('style', `transform:translate(160px,${currentMove % 2 == 0 ? 0 : 560}px);`);
                a.id = `3,${a.id.split(',')[1]}`;
                b.id = `2,${b.id.split(',')[1]}`;
            };
        };
    ...
    let p;
    if (oldColumn > 0 && oldLine > 0) p = document.getElementById(`${oldColumn},${oldLine}`);
    ...
    if (m !== '1/2-1/2' && m !== '1-0' && m !== '0-1' && !m.startsWith('O')) {
        if (p.classList[0] == 'Pawn' && `${parseInt(p.id.split(',')[0]) + 1},${parseInt(p.id.split(',')[1]) + (currentMove % 2 == 1 ? 1 : -1)}` == `${column},${line}` && !document.getElementById(`${parseInt(p.id.split(',')[0]) + 1},${parseInt(p.id.split(',')[1]) + (currentMove % 2 == 1 ? 1 : -1)}`) && document.getElementById(`${parseInt(p.id.split(',')[0]) + 1},${p.id.split(',')[1]}`)) {
            var z = document.getElementById(`${parseInt(p.id.split(',')[0]) + 1},${p.id.split(',')[1]}`)
            took++;
            pastPiece.taken = took;
            z.style.display = 'none';
            z.id = took.toString();
            pastPiece.ep = `${parseInt(p.id.split(',')[0]) + 1},${p.id.split(',')[1]}`
        } else if (p.classList[0] == 'Pawn' && `${parseInt(p.id.split(',')[0]) - 1},${parseInt(p.id.split(',')[1]) + (currentMove % 2 == 1 ? 1 : -1)}` == `${column},${line}` && !document.getElementById(`${parseInt(p.id.split(',')[0]) - 1},${parseInt(p.id.split(',')[1]) + (currentMove % 2 == 1 ? 1 : -1)}`) && document.getElementById(`${parseInt(p.id.split(',')[0]) - 1},${p.id.split(',')[1]}`)) {
            var z = document.getElementById(`${parseInt(p.id.split(',')[0]) - 1},${p.id.split(',')[1]}`)
            took++;
            pastPiece.taken = took;
            z.style.display = 'none';
    ...
    if (m[2] == '=') {
        let word;
        if (m[3] === 'B') word = 'Bishop';
        else if (m[3] === 'Q') word = 'Queen';
        else if (m[3] === 'N') word = 'Knight';
        else if (m[3] === 'R') word = 'Rook';
        p.classList[0] = word;
        p.src = `../Images/${word.toLowerCase()}_${p.classList[2].split('_')[0]}.png`;
        pastPiece.promotion = true;
    }
    lastMoves.push(pastPiece);
};
